...
- `src/services/learningPlanService.ts` 只计算当前周学习计划，不提前生成整条长期计划
- `src/services/adaptiveScenarioService.ts` 根据首页定位结果和上一日学习报告调整每日场景顺序
- `src/components/LearningCopilot.tsx` 把查词、题目解释、语法例句和反馈采集合并到学习过程内
...
定位结果会设置学习起点、学段版本、当前周每日任务重量、第一周方向和主要薄弱项。第一版使用本地规则评分，后续可以把同一接口接到 MiniMax/GPT 诊断。

当前每日计划规则：

- 小初版前两周每天 3 个真实场景，之后每两周加 1 个，直到每天 10 个。
- 高中版前两周每天 5 个真实场景，之后每两周加 1 个，直到每天 10 个。
- 每周根据最近完成量、生词积累和错误类型调整难度，但不改变原有场景池组合策略和词库范围。
- 系统只计算当前周和下一步提示，不一次性生成所有未来学习计划。
- 第一天会先插入与首页定位结果相关的 bridge 场景，再进入普通场景池。
- 第二天开始会先做上一日 warm-up，再把上一日的生词、语法和错误变成当天第一道复习迁移场景。

学习完成页现在必须给出可执行复习方案：当天激活词、语法例句、次日第一步复习提示，并允许低于自动触发阈值时也手动进入词汇复习。

每日还必须有独立词汇目标：系统从当天句式场景中抽取目标词，学习过程中显示“今日必须完成词”，句式任务完成后进行汉英互换小测。目标词、来源句和小测题都支持英文语音播放。小测包含英文选中文和中文拼写英文，初期拼写题给首尾字母提示；通过前不能进入下一天。

本地档案已经加入访问码保护：新建学习者需要设置本机访问码，切换受保护学习者时需要输入访问码，避免同一设备上的其他人随意进入别人的学习进度。当前访问码只用于本机 `localStorage` 防误选，不等同于云端身份认证。
